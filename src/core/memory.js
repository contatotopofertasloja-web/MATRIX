// src/core/memory.js
// Memória por JID (slots + timestamps).
// - TTL padrão: 7 dias
// - Backend: Redis se disponível (MATRIX_REDIS_URL|REDIS_URL), senão RAM
// - API estável: get(jid), set(jid, data), merge(jid, patch)
...
function startSweeper() {
  ...
  }, Math.min(TTL_MS, 60 * 60 * 1000)); // varredura a cada 1h (ou menos, se TTL < 1h)
}
...
// ===============
function fresh(obj) {
  return { ...(obj || {}), _expiresAt: Date.now() + TTL_MS };
}
function key(jid) {
  return `${redisPrefix}${jid}`;
}
...
export async function get(jid) {
  if (useRedis) {
    try {
      ...
      console.warn("[memory.get] redis:", e?.message || e);
      // fallback: RAM
    }
  }
  // RAM
...
export async function set(jid, data) {
  ...
  if (useRedis) {
    try {
      ...
      console.warn("[memory.set] redis:", e?.message || e);
      // fallback: RAM
    }
  }
  startSweeper();
...
// utilitário opcional: apaga memória de um JID (debug/ops)
export async function clear(jid) {
  if (useRedis) {
    try { await redis.del(key(jid)); return; } catch {}
  }
  store.delete(jid);
}

// utilitário opcional: troca TTL em runtime (para testes)
