...
import { callUser, setNameFromText } from "./_state.js";
...
const QUESTIONS = [
  { key: "tipo_cabelo",     q: "Seu cabelo é **liso**, **ondulado**, **cacheado** ou **crespo**?" }, // ← chave corrigida
...
function smartFill(state, text) {
  ...
  if (m && !state.tipo_cabelo) state.tipo_cabelo = m[1]; // ← salva na chave usada pelo estado atual
}
...
export default async function qualify(ctx) {
  ...
  // memoriza nome se o usuário se apresentar
  setNameFromText(state, text);
